 ...
 */
	public function testConstruct() {
		...
		$this->Socket->reset();
		$baseConfig = $this->Socket->config;
		$this->Socket->__construct('http://www.gerenciador-tarefas.org:23/');
		$baseConfig['host'] = $baseConfig['request']['uri']['host'] = 'www.gerenciador-tarefas.org';
		$baseConfig['port'] = $baseConfig['request']['uri']['port'] = 23;
		$baseConfig['request']['uri']['scheme'] = 'http';
		$this->assertEquals($this->Socket->config, $baseConfig);

		$this->Socket->reset();
		$this->Socket->__construct(array('request' => array('uri' => 'http://www.gerenciador-tarefas.org:23/')));
		$this->assertEquals($this->Socket->config, $baseConfig);
	}
 ...
 */
	public function testConfigUri() {
		$this->Socket->reset();
		$r = $this->Socket->configUri('https://bob:secret@www.gerenciador-tarefas.org:23/?query=foo');
		$expected = array(
			'persistent' => false,
			'host' => 'www.gerenciador-tarefas.org',
			'protocol' => 'tcp',
			'port' => 23,
			'timeout' => 30,
 ...
 */
	public function testRequest() {
		...
		$tests = array(
			array(
				'request' => 'http://www.gerenciador-tarefas.org/?foo=bar',
				'expectation' => array(
					'config' => array(
						'persistent' => false,
						'host' => 'www.gerenciador-tarefas.org',
					...
					'request' => array(
						...
						'header' => "Host: www.gerenciador-tarefas.org\r\nConnection: close\r\nUser-Agent: CakePHP\r\n",
						'raw' => "",
						'redirect' => false,
						'cookies' => array(),
						'proxy' => array(),
			...
			array(
				'request' => 'www.gerenciador-tarefas.org/?foo=bar'
			),
			array(
				'request' => array(
					'host' => '192.168.0.1',
					'uri' => 'http://www.gerenciador-tarefas.org/?foo=bar'
				),
				'expectation' => array(
					'request' => array(
						'uri' => array('host' => 'www.gerenciador-tarefas.org')
					),
					'config' => array(
						'request' => array(
							'uri' => array('host' => 'www.gerenciador-tarefas.org')
			...
			array(
				'request' => array('header' => array('Foo@woo' => 'bar-value', 'host' => 'foo.com'), 'uri' => 'http://www.gerenciador-tarefas.org/'),
				'expectation' => array(
					'request' => array(
						'header' => "Host: foo.com\r\nConnection: close\r\nUser-Agent: CakePHP\r\nFoo\"@\"woo: bar-value\r\n"
					),
			...
			array(
				'request' => array('header' => "Foo: bar\r\n", 'uri' => 'http://www.gerenciador-tarefas.org/search?q=http_socket#ignore-me'),
				'expectation' => array(
					'request' => array(
						'uri' => array(
							'path' => '/search',
			...
			array(
				'request' => array(
					'method' => 'POST',
					'uri' => 'http://www.gerenciador-tarefas.org/posts/add',
					'body' => array(
			...
			array(
				'request' => array(
					'method' => 'POST',
					'uri' => 'http://www.gerenciador-tarefas.org:8080/posts/add',
					'body' => array(
						'name' => 'HttpSocket-is-released',
						'date' => 'today'
					)
				...
				'expectation' => array(
					...
					'request' => array(
						...
						'header' => "Host: www.gerenciador-tarefas.org:8080\r\nConnection: close\r\nUser-Agent: CakePHP\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 38\r\n"
					)
				)
			),
			'reset10' => array(
			...
			array(
				'request' => array(
					'method' => 'POST',
					'uri' => 'https://www.gerenciador-tarefas.org/posts/add',
					'body' => array(
						'name' => 'HttpSocket-is-released',
						'date' => 'today'
					)
		...
		$this->Socket->reset();
		$request = array('method' => 'POST', 'uri' => 'http://www.gerenciador-tarefas.org/posts/add', 'body' => array('name' => 'HttpSocket-is-released', 'date' => 'today'));
 ...
 */
	public function testGetWithSchemeAndPort() {
		...
		$this->Socket->request($request);
		$this->assertContains('Host: gerenciador-tarefas.org:8080', $this->Socket->request['header']);
	}

/**
 * Test URLs like http://gerenciador-tarefas.org/index.php?somestring without key/value pair for query
 *
 * @return void
 */
 ...
 */
	public function testRequest2() {
		...
		$request = array('uri' => 'htpp://www.gerenciador-tarefas.org/');
 ...
 */
	public function testRequest3() {
		...
		$this->assertEquals($expected, $this->Socket->config['request']['cookies']['www.gerenciador-tarefas.org']);
 ...
 */
	public function testRequestWithResource() {
		...
		$result = (string)$this->Socket->request('http://www.gerenciador-tarefas.org/');
 ...
 */
	public function testRequestWithCrossCookie() {
		...
		$expected = array('www.gerenciador-tarefas.org' => array('foo' => array('value' => 'bar')));
		$this->Socket->request('http://www.gerenciador-tarefas.org/');
		$this->assertEquals($expected, $this->Socket->config['request']['cookies']);

		$serverResponse = "HTTP/1.x 200 OK\r\nSet-Cookie: bar=foo\r\nDate: Mon, 16 Apr 2007 04:14:16 GMT\r\nServer: CakeHttp Server\r\nContent-Type: text/html\r\n\r\n<h1>This is a test!</h1>";
		$this->Socket->expects($this->at(1))->method('read')->will($this->returnValue($serverResponse));
		$this->Socket->expects($this->at(2))->method('read')->will($this->returnValue(false));
		$this->Socket->request('http://www.gerenciador-tarefas.org/other');
		$this->assertEquals(array('foo' => array('value' => 'bar')), $this->Socket->request['cookies']);
		$expected['www.gerenciador-tarefas.org'] += array('bar' => array('value' => 'foo'));
		$this->assertEquals($expected, $this->Socket->config['request']['cookies']);
 ...
 */
	public function testRequestCustomResponse() {
		...
		$response = $this->Socket->request('http://www.gerenciador-tarefas.org/');
 ...
 */
	public function testProxy() {
		...
		$expected = "GET http://www.gerenciador-tarefas.org/ HTTP/1.1\r\nHost: www.gerenciador-tarefas.org\r\nConnection: close\r\nUser-Agent: CakePHP\r\n\r\n";
		$this->Socket->request('http://www.gerenciador-tarefas.org/');
		...
		$expected = "GET http://www.gerenciador-tarefas.org/bakery HTTP/1.1\r\nHost: www.gerenciador-tarefas.org\r\nConnection: close\r\nUser-Agent: CakePHP\r\n\r\n";
 ...
 */
	public function testUrl() {
		...
		$url = $this->Socket->url('www.gerenciador-tarefas.org');
		$this->assertEquals('http://www.gerenciador-tarefas.org/', $url);

		$url = $this->Socket->url('https://www.gerenciador-tarefas.org/posts/add');
		$this->assertEquals('https://www.gerenciador-tarefas.org/posts/add', $url);
		$url = $this->Socket->url('http://www.gerenciador-tarefas/search?q=socket', '/%path?%query');
		$this->assertEquals('/search?q=socket', $url);

		$this->Socket->config['request']['uri']['host'] = 'bakery.gerenciador-tarefas.org';
		$url = $this->Socket->url();
		$this->assertEquals('http://bakery.gerenciador-tarefas.org/', $url);

		$this->Socket->configUri('http://www.gerenciador-tarefas.org');
		$url = $this->Socket->url('/search?q=bar');
		$this->assertEquals('http://www.gerenciador-tarefas.org/search?q=bar', $url);
		...
		$this->assertEquals('http://www.gerenciador-tarefas.org/supersearch?q=bar', $url);
 ...
 */
	public function testBuildRequestLine() {
		...
		$r = $this->Socket->buildRequestLine(array('method' => 'GET', 'uri' => 'http://www.gerenciador-tarefas.org/search?q=socket'));
 ...
 */
	public function testParseUri() {
		...
		$uri = $this->Socket->parseUri('http://bob:foo123@www.gerenciador-tarefas.org:40/search?q=dessert#results');
		...
		$uri = $this->Socket->parseUri('http://www.gerenciador-tarefas.org/');
		$this->assertEquals($uri, array(
			'scheme' => 'http',
			'host' => 'www.gerenciador-tarefas.org',
			'path' => '/'
		...
		$uri = $this->Socket->parseUri('http://www.gerenciador-tarefas.org', true);
		$this->assertEquals($uri, array(
			'scheme' => 'http',
			'host' => 'www.gerenciador-tarefas.org',
			'port' => 80,
		...
		$uri = $this->Socket->parseUri('https://www.gerenciador-tarefas.org', true);
		$this->assertEquals($uri, array(
			'scheme' => 'https',
			'host' => 'www.gerenciador-tarefas.org',
			'port' => 443,
		...
		$uri = $this->Socket->parseUri('www.gerenciador-tarefas.org:443/query?foo', true);
		$this->assertEquals($uri, array(
			'scheme' => 'https',
			'host' => 'www.gerenciador-tarefas.org',
			'port' => 443,
		...
		$uri = $this->Socket->parseUri('http://www.gerenciador-tarefas.org', array('host' => 'piephp.org', 'user' => 'bob', 'fragment' => 'results'));
		$this->assertEquals($uri, array(
			'host' => 'www.gerenciador-tarefas.org',
			'user' => 'bob',
			'fragment' => 'results',
		...
		$uri = $this->Socket->parseUri('https://www.gerenciador-tarefas.org', array('scheme' => 'http', 'port' => 23));
		$this->assertEquals($uri, array(
			'scheme' => 'https',
			'port' => 23,
			'host' => 'www.gerenciador-tarefas.org'
		...
		$uri = $this->Socket->parseUri('www.gerenciador-tarefas.org:59', array('scheme' => array('http', 'https'), 'port' => 80));
		$this->assertEquals($uri, array(
			'scheme' => 'http',
			'port' => 59,
			'host' => 'www.gerenciador-tarefas.org'
		...
		$uri = $this->Socket->parseUri('http://www.gerenciador-tarefas.org/?param1=value1&param2=value2%3Dvalue3');
		$this->assertEquals($uri, array(
			'scheme' => 'http',
			'host' => 'www.gerenciador-tarefas.org',
			'path' => '/',
		...
		$uri = $this->Socket->parseUri('http://www.gerenciador-tarefas.org/?param1=value1&param2=value2=value3');
		$this->assertEquals($uri, array(
			'scheme' => 'http',
			'host' => 'www.gerenciador-tarefas.org',
			'path' => '/',
 ...
 */
	public function testBuildUri() {
		...
		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org'));
		$this->assertEquals('http://www.gerenciador-tarefas.org/', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'scheme' => 'https'));
		$this->assertEquals('https://www.gerenciador-tarefas.org/', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'port' => 23));
		$this->assertEquals('http://www.gerenciador-tarefas.org:23/', $r);

		$r = $this->Socket->buildUri(array('path' => 'www.google.com/search', 'query' => 'q=gerenciador-tarefas'));
		$this->assertEquals('http://www.google.com/search?q=gerenciador-tarefas', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'scheme' => 'https', 'port' => 79));
		$this->assertEquals('https://www.gerenciador-tarefas.org:79/', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'path' => 'foo'));
		$this->assertEquals('http://www.gerenciador-tarefas.org/foo', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'path' => '/foo'));
		$this->assertEquals('http://www.gerenciador-tarefas.org/foo', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'path' => '/search', 'query' => array('q' => 'HttpSocket')));
		$this->assertEquals('http://www.gerenciador-tarefas.org/search?q=HttpSocket', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'fragment' => 'bar'));
		$this->assertEquals('http://www.gerenciador-tarefas.org/#bar', $r);
		...
		$this->assertEquals('https://bob:secret@www.gerenciador-tarefas.org:25/cool?foo=bar#comment', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org', 'fragment' => 'bar'), '%fragment?%host');
		$this->assertEquals('bar?www.gerenciador-tarefas.org', $r);

		$r = $this->Socket->buildUri(array('host' => 'www.gerenciador-tarefas.org'), '%fragment???%host');
		$this->assertEquals('???www.gerenciador-tarefas.org', $r);

		$r = $this->Socket->buildUri(array('path' => '*'), '/%path?%query');
		$this->assertEquals('*', $r);

		$r = $this->Socket->buildUri(array('scheme' => 'foo', 'host' => 'www.gerenciador-tarefas.org'));
		$this->assertEquals('foo://www.gerenciador-tarefas.org:80/', $r);
	}
 ...
 */
	public function testParseQuery() {
		...
		$query = $this->Socket->parseQuery(array('framework' => 'gerenciador-tarefas'));
		$this->assertEquals(array('framework' => 'gerenciador-tarefas'), $query);

		$query = $this->Socket->parseQuery('');
		$this->assertEquals(array(), $query);

		$query = $this->Socket->parseQuery('framework=gerenciador-tarefas');
		$this->assertEquals(array('framework' => 'gerenciador-tarefas'), $query);
 ...
 */
	public function testBuildHeader() {
		...
		$r = $this->Socket->buildHeader(array('Host' => 'www.gerenciador-tarefas.org'));
		$this->assertEquals("Host: www.gerenciador-tarefas.org\r\n", $r);

		$r = $this->Socket->buildHeader(array('Host' => 'www.gerenciador-tarefas.org', 'Connection' => 'Close'));
		$this->assertEquals("Host: www.gerenciador-tarefas.org\r\nConnection: Close\r\n", $r);
